...
- Local notifications: settings `Notifications` toggle requests OS permissions once and persists to local storage; when enabled, the Game detail screen can schedule local alerts ~15 minutes before tipoff using `expo-notifications`.

### To-do (next steps)

1. **Calendar actions** — e.g. add-to-calendar or deep links from game rows (product-dependent).
2. **Polish / QA** — device testing with `EXPO_PUBLIC_API_URL`, reduce duplicate fetches after splash if desired (context or cache).
